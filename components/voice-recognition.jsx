  ...
  return (
    <div className="flex flex-col items-center">
      <div className={`relative ${isListening ? "animate-pulse" : ""}`}>
        {isListening && <div className="absolute inset-0 rounded-full bg-primary/20 animate-ping"></div>}
        <Button
          onClick={toggleListening}
          variant={isListening ? "default" : "outline"}
          size="lg"
          className={`rounded-full h-16 w-16 flex items-center justify-center shadow-lg ${
            isListening ? "bg-primary text-white" : "bg-muted text-primary hover:bg-primary/20"
          }`}
      ...
      <span className="mt-3 text-sm text-card-foreground">
